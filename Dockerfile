...
# then log output of file /code/bin/sslipd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/sslipd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/sslipd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/sslipd /usr/bin/sslipd
