...
HTTPS_GIT := github.com/robbedeboosere/rollchain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=rollchain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=sslipd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/sslipd ./cmd/sslipd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/sslipd.exe ./cmd/sslipd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/sslipd
...
draw-deps:
	...
	@goviz -i ./cmd/sslipd -d 2 | dot -Tpng -o dependency-graph.png
...
set-testnet-configs:
	sslipd config set client chain-id chainid-1
	sslipd config set client keyring-backend test
	sslipd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | sslipd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | sslipd keys add acc1 --recover`
